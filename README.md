# 🌐 BilingualSite - React i18n Portfolio Project

[![React](https://img.shields.io/badge/React-18-61DAFB?logo=react&logoColor=black)](https://reactjs.org/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.0-3178C6?logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![TailwindCSS](https://img.shields.io/badge/TailwindCSS-3.0-38B2AC?logo=tailwind-css&logoColor=white)](https://tailwindcss.com/)
[![i18next](https://img.shields.io/badge/i18next-23.0-26A69A?logo=i18next&logoColor=white)](https://www.i18next.com/)

A bilingual portfolio project focused on implementing **internationalization (i18n)**. Built with React, TypeScript, TailwindCSS, and i18next.

## ✨ Features

- 🌍 **Bilingual Support** - Seamless switching between two languages
- 🔧 **i18next Integration** - Professional internationalization library
- 📱 **Responsive Design** - Works on all devices with TailwindCSS
- 🟦 **TypeScript** - Type-safe codebase
- 📦 **Lightweight** - Pure React (no Next.js overhead)

## 🛠️ Tech Stack

- **Framework:** React 18
- **Language:** TypeScript
- **Styling:** TailwindCSS
- **i18n:** i18next + react-i18next

## 🚀 Installation & Running

**Prerequisites:** [Node.js](https://nodejs.org/) and [pnpm](https://pnpm.io/) 

```bash
# 1. Clone the repository
git clone https://github.com/ansariahmadreza/BilingualSite.git

# 2. Navigate into project folder
cd BilingualSite

# 3. Install dependencies
npm install
# or
pnpm install

# 4. Run development server
npm start
# or
pnpm start
